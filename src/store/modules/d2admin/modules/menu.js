...
function asignMenu (menu) {
  ...
  for (let item of menu) {
    ...
    if (item.pid === 0) {
      ...
      const parentRouter = menu.find(route => route.id === item.pid);
      
      if (parentRouter) {
        if (parentRouter.children) {
          parentRouter.children.push(item);
        } else {
          parentRouter.children = [item];
        }
      } else {
        console.error('未知路由：', item);
      }
    }
  }
...
export default {
  ...
  actions: {
    ...
    setAsideMenu ({state, commit}, {menu}) {
      console.log('开始设置菜单：', menu);
  ...
  mutations: {
    ...
    asideSet (state, menu) {
      ...
      localStorage.setItem('aside', JSON.stringify(menu));
    }
  }
}
